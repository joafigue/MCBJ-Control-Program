...
#+BEGIN_SRC python
  class iv_config(object):
      ...
      def get_measure_jv(self):
          return adwin.adwin_ADC(self.measure_jv.value)
      def get_end_jv(self):
...
#+BEGIN_SRC python
  class histogram_config(object):
      ...
      def get_break_wait(self):
          ...
          break_steps = adwin.adwin_ADC(break_speed/100) *1e-3 # in ms
...
#+BEGIN_SRC python
  class save_options(object):
      ...
      def update_config(self):
          ...
          self.config['SaveDir'] = self.save_dir.value
  ...
  #+BEGIN_SRC python  -n -r
    ...
    class integer_parameter(numerical_parameter):
        ...
        def update(self, new_val): # (ref:src-ip-validation)
            super(integer_parameter, self).update(val)
...
#+BEGIN_SRC python
  ...
  class bool_param(object):
      ...
      def print_param(self):#(ref:src-bool-print)
          print("{0} is = {1}".format(self._name, self.value))
...
#+BEGIN_SRC python :exports none
  """ Module ui_config - Provides the definitions and parameters for the UI
  
             This file provides a all the necessary helper
...
#+BEGIN_SRC python
  ...
  def ui_create_config_layout(ui_config_window):
      ...
      iv_param_layout = ui_iv_param_layout(ui_config_window)
      hist_param_layout = ui_hist_param_layout(ui_config_window)
      display_param_layout = ui_display_param_layout(ui_config_window)
      save_opts_layout = ui_save_opts_layout(ui_config_window)
      # Define the labels
...
#+BEGIN_SRC python :tangle no
  def measure_and_get_histogram(self):
      ...
      while self.process_running():
          ...
          if state == 1:
              print("Run number: {0} \nbreaking 1".format(state))
          if state == 2:
              print("Run number: {0} \nmaking 1".format(state))
      os.system('cls')
      self.stop_process()
...
*** TODO ADwin Histogram testing                                   :noexport:
    :PROPERTIES:
    :header-args+: :tangle adwin_histogram_test.py
    :END:
...
#+BEGIN_SRC python
  ...
  def save_data(*arg): 
      ...
      for i in range(0,len(arg[1])):
          ...
          for j in range (1,len(arg)): 
              temp = arg[j]
              if j == 1:
                  string = string +  "%5.8f\t" % (temp[i])
              else:
                  string = string +  "%5.8e\t" % (temp[i])
  ...
  def plot_traces(axis, D_breaking, G_breaking, D_making, G_making, dconfig):
      axis.clear()
      ...
      axis.set_ylim(dconfig.Gmin, dconfig.Gmax)
      axis.set_title("Conductance trace")
      axis.set_xlabel('Displacement (nm)')
      axis.set_ylabel('Conductance (G0)')
      axis.set_aspect(1)
      #axis.xticks(power(np.zeros(nGbins) + 10.0, range(-8, 2, 4)))
      #axis.yticks(range(-10, 1, 10))

  def plot_counts(axis, break_acc, make_acc, G_breaking, G_making, Gbins, Dbins):
  ...
  def plot_2D_histogram(axis, G_breaking, G_making, Gbins, D_breaking, Dbins, histo2D_breaking_sum):
      index=max(pl.find(G_breaking>0.5))
      D_breaking = D_breaking - D_breaking[index]
      Gbins_plot = Gbins[0:-1]
      Dbins_plot = Dbins[0:-1]

      # plot 2D histogram (3)
      tmp = pl.histogram2d(G_breaking, D_breaking, bins=(Gbins, Dbins))
      histo2D_breaking_sum = histo2D_breaking_sum + tmp[0]

      index1 = pl.find(min(abs(np.log10(Gbins) - -2 ))==(abs(np.log10(Gbins) - -2 )))
      index2 = pl.find(min(abs(np.log10(Gbins) - -6 ))==(abs(np.log10(Gbins) - -6 )))
      ...
      axis.clear()
      surf = axis.contourf(Dbins_plot, Gbins_plot, histo2D_breaking_sum, np.linspace(0, Cmax/2, 16))
      axis.set_yscale("log")
  ...
  save_dir = sconfig.get_save_dir()

  traces = dconfig.get_traces()
  nGbins = dconfig.get_nGbins()
  nDbins = dconfig.get_nXbins()
  xmin = dconfig.get_xmin()
  xmax = dconfig.get_xmax()
  Gmin = dconfig.get_Gmin()
  Gmax = dconfig.get_Gmax()



  ## initialize histogram
  Gbins = np.power(np.zeros(nGbins) + 10.0, np.linspace(np.log10(Gmin), np.log10(Gmax), nGbins)) # create conductance array
  Dbins = np.linspace(xmin, xmax, nDbins) # create displacement array

  histo1D_breaking_sum = np.zeros(nGbins-1)
  histo1D_making_sum = np.zeros(nGbins-1)
  histo2D_breaking_sum = np.zeros(shape=(nGbins-1, nDbins-1))
  ...
  break_acc = np.histogram(G_breaking, bins=Gbins)[0]
  make_acc = np.histogram(G_making, bins=Gbins)[0]
  for i in range(traces):
      break_histogram, make_histogram = adw.measure_and_get_histogram()
      if not adw.successful_measurement():
          continue
      D_breaking = break_histogram[0]
      G_breaking = break_histogram[1]
      ...
      plot_traces(ax1, D_breaking, G_breaking, D_making, G_making, dconfig)
      break_acc, make_acc = plot_counts(ax2, break_acc, make_acc, G_breaking, G_making, Gbins, Dbins)
      plot_2D_histogram(ax2, G_breaking, G_making, Gbins, D_breaking, Dbins, histo2D_breaking_sum)
      filename = save_dir + 'scan_jf' + date + '_' + "%1.0f.dat" % i
      save_file = open(filename, "w")
      #make_general_header(file)
      #make_piezo_header(file)
      save_data(save_file, D_breaking, G_breaking)
      save_data(save_file, D_making, G_making)
      save_file.close()
...
#+BEGIN_SRC python
  ...
  def adwin_convert_ms_to_cycles(time):
      ...
      wait_cycles = int(time*1e-3 /cycle_time ) # Because ms
      return wait_cycles
#+END_SRC
*** TODO Adwin reset
#+BEGIN_SRC python
...
def ADwin_boot():
  ...
  clean_adwin_driver = adwin_driver(1,"")
...
#+BEGIN_SRC basic
  ...
  DIM DATA_1[1048576] as float  'Break Conductances
  DIM DATA_2[1048576] as float  'Break Voltages
  DIM DATA_3[1048576] as float  'Make Conductances
  DIM DATA_4[1048576] as float  'Make Voltages
  ' Set DATA_10 as amplifier array
  DIM DATA_10[65536] as float
#+END_SRC
