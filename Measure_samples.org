...
- Getter function :: Defined in line [[(src-jv-get)]] extracts the value of the parameter. This is used as a generic interface available for every parameter
  ...
  class juncture_voltage:
      ...
      def update(self,val):#(ref:src-jv-update)
          if self.validate(val):
              self.voltage = val
      #############################################################
      ## @brief   Get the parameter value (voltage)
      #############################################################
      def get_value(self):#(ref:src-jv-get)
          return self.voltage
#+END_SRC
...
- Getter function :: Defined in line [[(src-ps-get)]] extracts the value of the parameter. This is used as a generic interface available for every parameter
  ...
  class piezo_speed: 
      ...
      def update(self,val): #(ref:src-ps-update)
          if self.validate(val):
              self.speed = val
      #############################################################
      ## @brief   Get the parameter value (speed)
      #############################################################
      def get_value(self):#(ref:src-ps-get)
          return self.speed
#+END_SRC
...
- Getter function :: Defined in line [[(src-tr-get)]] extracts the value of the parameter. This is used as a generic interface available for every parameter
  ...
  class traces:
      ...
      def update(self,aux_val):#(ref:src-tr-update)
          ...
          if self.validate(val):
              self.number = int(val)
      #############################################################
      ## @brief   Get the parameter value (traces)
      #############################################################
      def get_value(self):#(ref:src-tr-get)
          return self.number
#+END_SRC
...
#+BEGIN_SRC python
  ...
  def ui_basic_param_layout(window):
      ...
      traces_ed.setText(str(window.ui_config.config._b_params.traces.number))
