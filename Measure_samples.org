...
#+BEGIN_SRC basic
  ...
  DIM DATA_2[1048576] as float  'Break histogram
  DIM DATA_3[1048576] as float  'Make  histogram
  DIM DATA_4[1048576] as float  'Make  histogram
  ' Set DATA_10 as amplifier array
  DIM DATA_10[65536] as float
#+END_SRC
...
** TODO Adwin driver - program interface
   :PROPERTIES:
   :header-args: :tangle mudules/adwin_driver.py
...
As previously stated the global parameters hold information necessary for the ADwin instrument to work
...
The Adwin driver interface provides every functionality needed to interact with the ADwin instrument. 
...
#+BEGIN_SRC python
  ...
  class adwin_driver:
      def __init__(self):
          self._programs_subdir="adwin_programs"
          self.adw = ADwin.ADwin(ADW_GPARAM.ADDRESS,1)
          self.adw.Boot(ADW_GPARAM.BOOT_SCRIPT_DFLT)
          self.load_iv_process()
          self.load_histogram_process()
          self.configure_iv_process()
          self.configure_hist_process()        
      def load_iv_process(self):
          root_path = utl.get_script_root_path()
          program_path = os.path.join(root_path,self._programs_subdir)
          iv_filename = "iv_measurement_process.T%d" % ADW_GPARAM.IV_PROCESS
          iv_process_file = os.path.join(program_path,iv_filename)
          if os.path.isfile(iv_process_file):
              self.adw.Load_Process(iv_process_file)
          else :
              raise Exception("Bad file, check %s" %iv_process_file)
      def load_histogram_process(self):
          root_path = utl.get_script_root_path()
          program_path = os.path.join(root_path,self._programs_subdir)
          hist_filename = "hist_measurement_process.T%d" % ADW_GPARAM.HIST_PROCESS
          hist_process_file = os.path.join(program_path,iv_filename)
          if os.path.isfile(hist_process_file):
              self.adw.Load_Process(hist_process_file)
          else :
              raise Exception("Bad file, check %s" %hist_process_file)
      def configure_iv_process(self):
          self.adw.Set_Processdelay(ADW_GPARAM.IV_PROCESS, process_delay)
          self.adw.SetDataFloat(log_array, ADW_APARAM.LOG_AMPL,1,len(log_array))
          self.adw.Set_Par(ADW_IV_PARAM.START_JV, start_jv)
          self.adw.Set_Par(ADW_IV_PARAM.MEASURE_JV, start_jv)
          self.adw.Set_Par(ADW_IV_PARAM.END_JV, start_jv)
          self.adw.Set_Par(ADW_IV_PARAM.WAIT, start_jv)
          self.adw.Set_Par(ADW_IV_PARAM.AVG, start_jv)
          self.adw.Set_Par(ADW_IV_PARAM.MAX_DATA, start_jv)        
      def configure_hist_process(self):
          self.adw.Set_Processdelay(ADW_GPARAM.HIST_PROCESS, process_delay)
          self.adw.SetDataFloat(log_array, ADW_APARAM.LOG_AMPL,1,len(log_array))
          self.adw.Set_Par(ADW_HIST_PARAM.START_JV, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.MEASURE_JV, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.END_JV, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.BREAK_WAIT, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.MAKE_WAIT, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.AVG, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.I_MIN_BRK, start_jv)
          self.adw.Set_Par(ADW_HIST_PARAM.I_MAX_BRK, start_jv)
#+END_SRC
