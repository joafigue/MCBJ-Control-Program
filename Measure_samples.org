...
#+BEGIN_SRC python
  ...
  class adwin_iv_config:
      def __init__(self, iv_conf):
          self.log_array = iv_conf.log_ampl.get_calibration()
          self.start_jv = convert_V_to_bin(iv_conf.start_jv)
          self.measure_jv = convert_V_to_bin(iv_conf.measure_jv.value)
          self.stop_jv = convert_V_to_bin(iv_conf.stop_jv)
          self.avg_points = iv_conf.avg_pints.value
          ...
          self.wait_stabilize = int(iv_conf.wait*1e-3 /cycle_time ) # Because ms
...
#+BEGIN_SRC python
    ...
    class adwin_hist_config:
        def __init__(self, hist_conf):
            self.log_array = hist_conf.log_ampl.get_calibration()
            self.start_jv = convert_V_to_bin(hist_conf.start_jv)
            self.measure_jv = convert_V_to_bin(hist_conf.measure_jv.value)
            self.stop_jv = convert_V_to_bin(hist_conf.stop_jv)
            self.avg_points = hist_conf.avg_pints.value
            # To determine the wait cycles we need the cycle time. (10^5)
            cycle_time = ADW_GCONST.HIGH_PERIOD * ADW_GCONST.PROCESS_DELAY
            # First we convert speed to steps/seg. Factor is due to adwin range
            # between [-10,10], so 1000 [V] at piezo is 10 [V] in adwin
            brk_steps_seg = convert_V_to_bin(hist_conf.break_speed.value/100)
            mk_steps_seg = convert_V_to_bin(hist_conf.make_speed.value/100)
            # Example 10 steps/s => 10^4 wait cycles as cycle time is 10^5
            self.break_wait = 1/(brk_steps_seg * cycle_time)
            self.make_wait = 1/(mk_steps_seg * cycle_time)
            self.I_break_end = hist_config.G_break_end*hist_conf.measure_jv.value
            self.I_make_end = hist_config.G_make_end*hist_conf.measure_jv.value
#+END_SRC
...
#+BEGIN_SRC python
  class display_config:
      ...
      def update_config_with_data(data):
          self.xmin.update_or_dftl(data.get('xmin'))
          self.xmax.update_or_dftl(data.get('xmax'))
          self.Gmin.update_or_dftl(data.get('Gmin'))
          self.Gmax.update_or_dftl(data.get('Gmax'))
          self.nGbins.update_or_dftl(data.get('nGbins'))
          self.nDbins.update_or_dftl(data.get('nDbins'))

          self.update_config()

  class display_config_values:
      def __init__(self, display_conf):
          self.xmin = display.conf.xmin.value
          self.xmax = display_conf.xmax.value
          self.Gmin = display_conf.Gmin.value
          self.Gmax = display_conf.Gmax.value
          self.nGbins = display_conf.nGbins.value
          self.nDbins = display_conf.nDbins.value
#+END_SRC
...
***** DONE Configuration Files specification
 ...
 #+END_SRC
***** TODO Configuration Files interface
      :PROPERTIES:
      :header-args+: :tangle   modules/config_interface.py
      :header-args+: :noweb    tangle
      :END:
 ...
 # Yaml is stored as python dictionaries.
 #+BEGIN_SRC python
   #################################################################
   ## @file    Configuration_parser.py
   #  @author  Joaquin Figueroa
   #  @date    Wed Jan 11 11:02:47 CLST 2017
   #  @brief   Parse Yaml configuration file and builds configs
   #
   #  @details This file uses PyYaml to parse the configuration file
   #           and builds the necessary configurations for each
   #           run type.
   #           Then we return the collection of configurations
   #################################################################
   import yaml
 #+end_src
 ...
 #+begin_src python
   ...
   def yaml_extract_juncture_voltage(data):
       General_config = data['General Configuration']
       new_jv_val = General_config['JunctureVoltage']
       jv = juncture_voltage()
       if !jv.validate(new_jv_val):
           print("ERROR: JV value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(jv._min, jv._max))
       jv.update(new_jv_val)
       return jv

   def yaml_extract_save_dir(data):
       General_config = data['General Configuration']
       target_dir = General_config['SaveDir']
       save_dir = data_dir()
       if !save_dir.validate(target_dir):
           print("Directory does not exists")
           raise
       save_dir.update(target_dir)
       return save_dir

   def yaml_extract_avg_points(data):
       Histo_config = data['Histogram Configuration']
       new_avg = Histo_config['AveragePoints']
       avg = average_points()
       if !avg.validate(new_avg):
           print("ERROR: AVG value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(avg._min, avg._max))
       avg.update(new_avg)
       return avg


   def yaml_extract_break_speed(data):
       Histo_config = data['Histogram Configuration']
       new_speed = Histo_config['BreakSpeed']
       speed = piezo_break_speed()
       if !speed.validate(new_avg):
           print("ERROR: Break Speed value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(speed._min, speed._max))
       speed.update(new_speed)
       return speed
   ...
   def yaml_extract_make_speed(data):
       Histo_config = data['Histogram Configuration']
       new_speed = Histo_config['MakeSpeed']
       speed = piezo_make_speed()
       if !speed.validate(new_avg):
           print("ERROR: Make Speed value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(speed._min, speed._max))
       speed.update(new_speed)
       return speed


   def yaml_extract_num_traces(data):
       Histo_config = data['Histogram Configuration']
       new_traces = Histo_config['NumberTraces']
       traces = number_traces()
       if !traces.validate(new_traces):
           print("ERROR: Num_traces value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(traces._min, traces._max))
       traces.update(new_speed)
       return traces


   def yaml_extract_xmin(data):
       config = data['Presentation Configuration']
       new_xmin = config['xmin']
       xmin = xmin()
       if !xmin.validate(new_xmin):
           print("ERROR: xmin value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(xmin._min, xmin._max))
       xmin.update(new_xmin)
       return xmin

   def yaml_extract_xmax(data):
       config = data['Presentation Configuration']
       new_xmax = config['xmax']
       xmax = xmax()
       if !xmax.validate(new_xmax):
           print("ERROR: xmax value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(xmax._min, xmax._max))
       xmax.update(new_xmax)
       return xmax


   def yaml_extract_Gmin(data):
       config = data['Presentation Configuration']
       new_Gmin = config['Gmin']
       Gmin = Gmin()
       if !Gmin.validate(new_Gmin):
           print("ERROR: Gmin value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(Gmin._min, Gmin._max))
       Gmin.update(new_Gmin)
       return Gmin


   def yaml_extract_Gmax(data):
       config = data['Presentation Configuration']
       new_Gmax = config['Gmax']
       Gmax = Gmax()
       if !Gmax.validate(new_Gmax):
           print("ERROR: Gmax value out of range. Using default")
           print("Range min ={0}, Max ={1}".format(Gmax._min, Gmax._max))
       Gmax.update(new_Gmax)
       return Gmax
   ...
   # testing
   if __name__ == "__main__":
       filepath = "Measurement_deafult_config.yaml"
...
#+BEGIN_SRC python
  ...
  class adwin_iv_driver(adwin_driver):
      ...
      def load_process(self):
          ...
          super(adwin_iv_driver).load_process(iv_filename)
      #############################################################
      ## @brief   Starts the measurement process
      #############################################################
...
#+BEGIN_SRC python
  ...
  class adwin_hist_driver(adwin_driver):
      ...
      def configure_hist_process(self):
          ...
          self.adw.Set_Par(ADW_HIST_CONST.I_MIN_BRK, config.I_break_end)
          self.adw.Set_Par(ADW_HIST_CONST.I_MAX_BRK, config.I_make_end) # Unsure
...
#+BEGIN_SRC python
  ...
  def convert_V_to_bin(V):
      # converts ADC/DAC voltage to bin number, given the voltage range and the ADW_GCONST.RESOLUTION (in bits)
      step=2*ADW_GCONST.OUTPUT_RANGE/(2**ADW_GCONST.RESOLUTION-1)
      voltage=np.arange(-ADW_GCONST.OUTPUT_RANGE, ADW_GCONST.OUTPUT_RANGE+step, step)
      try:
          l = len(V)
      ...
      return N_bin, convert_bin_to_V(N_bin, ADW_GCONST.OUTPUT_RANGE, ADW_GCONST.RESOLUTION)
#+END_SRC
