...
Now there is an additional requirement for the UI, which is the interface by itself, thus the UI module can be thought as the result of 3 pieces.
...
**** DONE Parameters definitions
Every configurable parameter, requires its definition, and the ability to modify it properly, the following shows the different user availbale parameters
...
#+BEGIN_SRC python -n -r
  ...
  #           corresponding interfaces.
  #############################################################
  class piezo_make(numerical_parameter):
      #############################################################
      ## @brief   Initilaization code
      #############################################################
      def __init__(self):
...
- General Parameters :: These are variables that should be set by the user, but can apply to any process associated with the measurement, for example the juncture voltage
...
#+BEGIN_SRC yaml
  ...
  Histogram Configuration :
    ...
    NumberTraces : 500            # Number of histograms
...
#+BEGIN_SRC python
  ...
  def convert_v_to_bin(V):
      output_range = ADW_GPARAM.OUTPUT_RANGE
      resolution = ADW_GPARAM.RESOLUTION
      bin_v,real_v =convert_V_to_bin(V,output_range,resolution)
      return real_v

  def wait_from_speed(speed):
      mod_speed = speed/100
      scanrate = 1/ADW_GPARAM.PROCESS_DELAY
      steps = 2*ADW_GPARAM.OUTPUT_RANGE/ 2**ADW_GPARAM.RESOLUTION
...
#+BEGIN_SRC python
  def convert_bin_to_V(N_bin):
      ...
      voltage = np.arange(-ADW_GPARAM.OUTPUT_RANGE, ADW_GPARAM.OUTPUT_RANGE+step, step)
      try:
          N_bin = int(N_bin)
          data = voltage[N_bin]
      except:
