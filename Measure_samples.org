...
****** TODO Data directory
The program provides the user with the ability to store the Data in a directory of his choice, which is stored as a string, however the default directory is chosen from  the location of program main file, so that path is analyzed and also stored for future reference in this parameter.
...
#+BEGIN_SRC python -n -r
  ...
  class ui_config_window(QtGui.QWidget):
      ...
      def initUI(self):    #(ref:fn-initUI)
          self.ui_config = modules.ui_config.UI_CONFIG() #(ref:ui-config)
...
#+BEGIN_SRC python -r -n
  ...
  def ui_create_buttons_layout(widget):
      ...
      quit_button.clicked.connect(
          lambda: widget.close_with_cmd(modules.ui_config.UI_CMD.EXIT)) (ref:lambda_btn)
...
#+BEGIN_SRC python
  ...
  #           path from the script_root, it's return
  #############################################################
...
* Faulhaber Driver
The Faulhaber motor is used to perform the initial breaking of the juncture as it has a far larger dynamic range than possible with a single piezoelectric actuator.  
