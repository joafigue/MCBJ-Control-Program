...
*** Adwin IV - variable Declarations
...
**** Adwin IV - Event set juncture voltage
     :PROPERTIES:
...
**** Adwin IV - Event stabilize system
     :PROPERTIES:
     :header-args:  :tangle   no
     :END:
...
#+END_SRC
**** Adwin IV - Event reset juncture voltage
     :PROPERTIES:
     :header-args:  :tangle   no
     :END:
...
** Adwin Measure - Generate piezo histogram
   :PROPERTIES:
...
*** Adwin histogram - measurement header
As in many programs, a header is required. In the Adwin case, this is necessary  to set the process parameters, such as the process number and the compiler variables.
...
#+BEGIN_SRC basic :exports none
  ...
  ' PAR_3 -> Juncture voltage at end (off)
  ' ---- Current Measurement ----
  ' PAR_7 -> Stabilizing time for breaking
  ' PAR_8 -> Stabilizing time for making
  ' PAR_9 -> Number of data point to average for each measurement
  ' ---- Measurement Modifiers ----
...
*** Adwin histogram - variable Declarations
In an Adwin program the local variables must be declared before the =INIT= block. This program defines several internal variables to deal with the state.
#+CAPTION: Variable declarations
...
**** Adwin histogram - Event set juncture voltage
     :PROPERTIES:
     :header-args:  :tangle   no
     :END:
...
**** Adwin histogram - Event break histogram 
     :PROPERTIES:
     :header-args:  :tangle   no
     :END:     
...
**** Adwin histogram - Event make histogram
     :PROPERTIES:
     :header-args:  :tangle   no
     :END:
...
#+BEGIN_SRC basic
  SELECTCASE make_hist_flag
    CASE 0 ' Move piezo to new point
      makewait = 0
      avgcounter = 0
      make_acc = 0
      dec(PAR_1)
    ...
    CASE 3  ' Store average in array
      avgcurrent = make_acc / PAR_9
      ...
      ENDIF
#+END_SRC
**** Adwin histogram - Event reset juncture voltage
     :PROPERTIES:
     :header-args:  :tangle   no
     :END:
...
** Adwin driver - program interface
