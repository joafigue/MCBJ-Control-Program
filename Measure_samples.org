...
It's clear that most parameters are numerical, therefore it makes sense to define a class which encapsulates the common functionality for the numerical parameters. This class is defined in [[Numerical Parameters definition]]

****** TODO Numerical Parameters definition
...
- Initialization function :: The function defined in line [[(np-init)]], provide the initialization code for a generic numerical parameter, which is defined by the following parameters:
...
The number of traces defined in [[src-config-traces-class]] is the parameter that controls the number of measurement cycles excecuted using the piezoelectric actuator, where a trace is a full cycle going from closed to open and back. The traces class is a specialization of the Numerical parameter class described in [[Numerical Parameters definition]], with the following modifications:
- Values must be integers :: This class must have all its values as integers, thus the default values and other parameters are redundantly cast as integers.
- Validation Function :: Defined in line [[(ref:src-tr-validation)]], this function also confirms that the parameter is an integer
...
#+BEGIN_SRC python -n -r
  ...
  class traces(numerical_parameter):
        ...
        #############################################################
        def validate(self, val):#(ref:src-tr-validation)
...
#+BEGIN_SRC python -n -r
  ...
  class paths:
      ...
      def validate(self, new_path):#(ref:src-dd-validation)
          return os.path.is_dir(new_path)
      #############################################################
...
#+BEGIN_SRC python
  ...
  def ui_basic_param_layout(window):
      ...
      dir_label = QtGui.QLabel(window.ui_config.config._b_params.paths.data_dir)
      ...
      jv_ed.setText(str(window.ui_config.config._b_params.juncture.value))
      ps_ed.setText(str(window.ui_config.config._b_params.piezo_speed.value))
      tr_ed.setText(str(window.ui_config.config._b_params.traces.value))
  ...
  def showDialog(window,dir_label):
      fname = QtGui.QFileDialog.getExistingDirectory(window, 'Open file',
              window.ui_config.config._b_params.paths.data_dir)
      if(fname):
              window.ui_config.config._b_params.paths.update(fname)
              dir_label.setText(window.ui_config.config._b_params.paths.data_dir)
              print(window.ui_config.config._b_params.paths.data_dir)
