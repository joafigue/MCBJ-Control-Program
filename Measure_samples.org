...
- ADDRESS :: The addres is the associated to the port used to communicate with the instrument.
...
#+BEGIN_SRC python
  ...
  class adwin_driver:
      ...
      def analyze(self):
          ...
          iv_status = self.Process_Status(ADW_GPARAM.IV_PROCESS)
          hist_status = self.Process_Status(ADW_GPARAM.HIST_PROCESS)
          print("Version %s " % name)
          print("Processor type: %s " % name)
          print_status(iv_status, ADW_GPARAM.IV_PROCESS)
          print_status(hist_status, ADW_GPARAM.HIST_PROCESS)
      #asdf, probably util
