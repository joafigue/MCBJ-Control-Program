...
** Adwin measure I/V process
   :PROPERTIES:
   :tangle:   adwin_programs/iv_measurement_process.bas
   :noweb:    tangle
   :comments: noweb
   :mkdirp:   yes
   :END:
...
#+BEGIN_SRC basic
  START_CONV(00011b)
  WAIT_EOC(00011b)
  current1_acc = totalcurrent1 + READADC(1)
  current2_acc = totalcurrent2 + READADC(2)
  current1_ampl_acc = totalcurrent3 + DATA_10[READADC(1)+1]
  current1_ampl_acc = totalcurrent4 + DATA_10[READADC(2)+1]
...
The interface to the Faulhaber motor is in \([rpm]\), however the magnitude has little meaning in the experiment, as the measured quantity is the displacement which is in \([\mu m]\), therefore conversion functions are used to move from one measurement to the other. Equation [[eq-conversion-rmp-ums]] shows the conversion formula using the motor *gearbox* and the *axis pitch*.
